import { useGSAP } from '@gsap/react';
import React, { useContext, useRef } from 'react'
import { NavbarColorContext, NavbarContext } from '../../context/NavContext';
import { Link, useLocation } from 'react-router-dom';
import SimpleNavbar from './SimpleNavbar';

const Navbar = () => {
  const [navOpen,setNavOpen] = useContext(NavbarContext)
  const [navColor,setNavColor] = useContext(NavbarColorContext);
  const path = useLocation().pathname;
  const navGreenRef1 = useRef(null);
  const navGreenRef2 = useRef(null);
  const navGreenRef3 = useRef(null);

  return (
    <div className='z-4 flex fixed top-0 w-full items-start justify-between'>
        <div className='lg:p-5 p-2'>
          <div className='lg:w-30 w-20'>
            <Link to="/">
            <svg xmlns="http://www.w3.org/2000/svg" className='w-full' viewBox="0 0 103 44">
              <path fill={navColor} fillRule="evenodd" d="M35.1441047,8.4486911 L58.6905011,8.4486911 L58.6905011,-1.3094819e-14 L35.1441047,-1.3094819e-14 L35.1441047,8.4486911 Z M20.0019577,0.000230366492 L8.83414254,25.3433089 L18.4876971,25.3433089 L29.5733875,0.000230366492 L20.0019577,0.000230366492 Z M72.5255345,0.000691099476 L72.5255345,8.44846073 L94.3991559,8.44846073 L94.3991559,16.8932356 L72.5275991,16.8932356 L72.5275991,19.5237906 L72.5255345,19.5237906 L72.5255345,43.9274346 L102.80937,43.9274346 L102.80937,35.4798953 L80.9357483,35.4798953 L80.9357483,25.3437696 L94.3996147,25.3428482 L94.3996147,16.8953089 L102.80937,16.8953089 L102.80937,0.000691099476 L72.5255345,0.000691099476 Z M-1.30398043e-14,43.9278953 L8.78642762,43.9278953 L8.78642762,0.0057591623 L-1.30398043e-14,0.0057591623 L-1.30398043e-14,43.9278953 Z M58.6849955,8.4486911 L43.1186904,43.9274346 L52.3166592,43.9274346 L67.9877996,8.4486911 L58.6849955,8.4486911 Z M18.4688864,25.3437696 L26.7045278,43.9278953 L36.2761871,43.9278953 L28.1676325,25.3375497 L18.4688864,25.3437696 Z"></path>
            </svg>
            </Link>
          </div>
        </div>
        {
          path === "/" ? (
            <SimpleNavbar setNavOpen={setNavOpen}/>
          ):(
            <>
            <div className="lg:flex hidden flex-row">
              <Link to="/projects"><div 
              onMouseEnter={()=>{
              navGreenRef1.current.style.height = '100%'
              }} 
              onMouseLeave={()=>{
              navGreenRef1.current.style.height='0%'
              }}
              className='lg:h-14 h-12 bg-black relative lg:w-[20vw] w-44 text-white font-semibold text-2xl uppercase cursor-pointer'>
              <div ref={navGreenRef1} className='bg-[#D3FD50] transition-all absolute top-0 h-full w-full'></div>
                  <div className='relative flex flex-col lg:gap-2 gap-1 items-start pl-2 justify-center h-full text-start'>
                      Projets
                  </div>
              </div></Link>
              <Link to="/agence">
              <div 
              onMouseEnter={()=>{
              navGreenRef2.current.style.height = '100%'
              }} 
              onMouseLeave={()=>{
              navGreenRef2.current.style.height='0%'
              }}
              className='lg:h-24 h-12 bg-black relative lg:w-[32vw] w-44 text-white font-semibold text-2xl uppercase text-center cursor-pointer'>
              <div ref={navGreenRef2} className='bg-[#D3FD50] transition-all absolute top-0 h-full w-full'></div>
                  <div className='relative flex flex-col lg:gap-2 gap-1 items-start pl-2 justify-center h-full lg:pt-10'>
                      Agence
                  </div>
              </div></Link>
              <div 
              onMouseEnter={()=>{
              navGreenRef3.current.style.height = '100%'
              }} 
              onMouseLeave={()=>{
              navGreenRef3.current.style.height='0%'
              }}
              onClick={()=>{
              setNavOpen(true)
              }}
              className='lg:h-36 h-12 bg-black relative lg:w-[16vw] w-44 text-white font-semibold text-2xl uppercase text-center cursor-pointer'>
              <div ref={navGreenRef3} className='bg-[#D3FD50] transition-all absolute top-0 h-full w-full'></div>
                  <div className='relative flex flex-col lg:gap-2 gap-1 items-start pt-10 justify-center h-full pl-2'>
                      <div className='lg:w-18 w-12 h-0.5 bg-white'></div>
                      <div className='lg:w-10 w-6 h-0.5 bg-white'></div>
                      Menu
                  </div>
              </div>
            </div>
            <div className="block lg:hidden">
              <SimpleNavbar setNavOpen={setNavOpen}/>
            </div>
            </>
          )
        }
    </div>
  )
}

export default Navbar
